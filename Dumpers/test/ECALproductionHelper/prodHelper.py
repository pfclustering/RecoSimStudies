...
if __name__ == "__main__":
  ...
  if not opt.doskipdumper:
    ...
    template_dumper = [
      ...
      'if [ "$USER" == "anlyon" ] ; then ',
        'cmsRun ../../python/Cfg_DeepClusteringDumper_cfg.py outputFile=/work/anlyon/dumpedFiles/{pl}.root inputFiles_load=../../data/samples/{pl}.txt'.format(pl=prodLabel),
      'else',
        'cmsRun ../../python/Cfg_DeepClusteringDumper_cfg.py outputFile=../../test/outputfiles/{pl}.root inputFiles_load=../../data/samples/{pl}.txt'.format(pl=prodLabel),
      'fi',
      'DATE_END=`date +%s`',
      'echo ""',
      '',
  ...
  # add the dumper part
  if not opt.doskipdumper:
    sbatch_command_dumper = 'jid_d=$(sbatch -p wn --account=cn-test -o logs/dumper.log -e logs/dumper.log --job-name=dumper_{pl} {t} --ntasks=1 --dependency=afterany{dd} launch_dumper.sh)'.format(pl=prodLabel,t='--time=0-03:59',dd=dumper_dependencies)
